...
# ╔═╡ 0f6e6fc9-aa8d-4dc2-879f-bdca61d30e0d
begin
	bin_positions = data.x
	bin_contents = data.y
end

# ╔═╡ 8dc24691-2769-4f06-94a0-469aa0a9d7ef
...
# ╔═╡ 30ff5c7d-3da5-44cb-84af-4f1426237c34
md"""
For the fit, the resonance peak was modeled with a Breit-Wigner function and the smooth background was approximated by a linear function. This choice was motivated by the shape of the given mass distribution, where the background appears approximately linear in the considered mass range.

The total fit model is therefore

``
f(x)=A\frac{\Gamma^2/4}{(x-m_0)^2+\Gamma^2/4}+(a+bx)
``
"""
...
p0 = [
    10000.0,   # A  -> peak amplitude
    3100.0,    # m0 -> resonance mass
    50.0,    # Γ  -> width
    3000.0,    # a  -> background offset
    0.0       # b  -> background slope
]

# ╔═╡ c8001dd2-0b74-47d0-94a4-67661df71a5f
begin
	fit = curve_fit(model, bin_positions, bin_contents, p0)
		
	p_fit = coef(fit)
	
	#fit data
	xfit = range(minimum(bin_positions),
		                maximum(bin_positions),
		                length=1000)
		
	yfit = model(xfit, p_fit)
	
end

# ╔═╡ 38eb45a9-e8d8-476c-8163-b3235b8ad479
begin
	    ...
	    scatter(
	        ...
	        xlabel="Mass (MeV/c²)",
	        ylabel="Counts",
	        title="Breit-Wigner fit",
	        ms=3,
			ylims=(0, maximum(bin_contents) * 1.1)
	    )
...
# ╔═╡ 5d092878-676d-4339-9901-4330ecdb437a
md"""
* For the continuous case, the cumulative distribution function (CDF) is obtained from the normalized integral of the probability density function (PDF).

```math
CDF(x) =
\frac{
\int_{x_{\min}}^{x} f(x')\,dx'
}{
...
# ╔═╡ 0a281e3a-8099-4ee2-9ba3-f611f784aa66
begin
	# construct CDF
    cdf_fit = cumsum(yfit ./ sum(yfit))
    ...
    plot(
		...
		fillrange = 0,
    	fillcolor = :lightblue,
        title="CDF from fitted PDF",
        label=false
    )
end

# ╔═╡ e5485f89-a536-4cb1-a955-896cfabb913e
fitted_pdf(x) = model(x, p_fit)

# ╔═╡ d13de820-51e0-4287-b39a-fbd3b64e05a8
num_samples = 100000
...
# ╔═╡ 089bca4d-f579-4b09-8e91-e58e8274d2b8
begin
    histogram(
        accepted_samples_hm_fit,
        bins=62,
        normalize=false,
        xlabel="Mass (MeV/c²)",
        ylabel="Counts",
    ...
    bin_width = (maximum(xfit) - minimum(xfit)) / 62
    scale = length(accepted_samples_hm_fit) * bin_width / sum(yfit .* step(xfit))
...
# ╔═╡ 4c5abc4e-696f-426a-8ab0-f6cddb0183f3
eff_hm_fit = length(accepted_samples_hm_fit) / num_samples
...
manifest_format = "2.0"
project_hash = "0702274e753fb647483bf88a4bc1e70f2cbff773"
